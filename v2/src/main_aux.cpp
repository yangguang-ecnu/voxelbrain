...
//	glPointSize(POINTSIZE*(float)width/(float)3650/zoom);
	...
	glTranslatef(-120.0f, -128.0f, -80.0f);
...
void main_module::crossection_plane(const V3i & dx, const V3i & dy, int  xpos, int  ypos, int size, int color){
	bool section_border;
	// area:
	const int xraySize = size;
	...
	//find teh appropriate raster position
	V3f point2d = GetOGLPos(xpos, ypos+xraySize, false);
	V3f vdir = GetOGLDirection();
        vdir /= vdir.length()*100;
	point2d+=vdir;

	glDisable(GL_DEPTH_TEST);
	//test cross for every raster pos.
	/*	glBegin(GL_LINES);
	  glVertex3f(point2d.x-0.1f, point2d.y, point2d.z);
	  glVertex3f(point2d.x+0.1f, point2d.y, point2d.z);
	  glVertex3f(point2d.x, point2d.y-0.1f, point2d.z);
	  glVertex3f(point2d.x, point2d.y+0.1f, point2d.z);
	  glVertex3f(point2d.x, point2d.y, point2d.z-0.1f);
	  glVertex3f(point2d.x, point2d.y, point2d.z+0.1f);
	  glEnd();
	*/
	  //do actual drawing
...
Point calculate_point(raw_volume & vol, V3i & w){
  ...
  V3i cur;
  float max = 0;
  for(cur.x = -4; cur.x <= 4; cur.x++) //loop over
    for(cur.y = -4; cur.y <= 4; cur.y++)
      for(cur.z = -4; cur.z <= 4; cur.z++)
	if((cur.length2() < 4*4) && (17.0<vol(w.x+cur.x, w.y+cur.y, w.z+cur.z))){ //within a sphere
...
bool is_border(const raw_volume & vol, V3i cur){
